...
def test_runpod_low_time_high_quality_configs_are_loadable() -> None:
    ...
    assert train.batch_size == 1024
    assert train.batch_size * train.steps >= pipeline.min_replay_samples * 16

    assert arena.device == "cuda"
    assert arena.max_turns == pipeline.self_play.max_turns
    assert arena.gumbel_simulations == 256
    assert arena.gumbel_max_considered_actions == 64
    assert arena.gumbel_simulations >= pipeline.self_play.gumbel_simulations
